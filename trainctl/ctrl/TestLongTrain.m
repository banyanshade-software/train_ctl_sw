...
- (void) test2
{
    tconf->trainlen_left_cm = 0;
    tconf->trainlen_right_cm = 19;
    ctrl3_init_train(0, &tvars, szero, 900, 1);
    //tvars._curposmm = 900;
    int16_t remain = -1;
    lsblk_num_t r[4] = {-1,-1,-1,-1};
...
- (void) test_chk_front_right
{
    ...
    ctrl3_init_train(0, &tvars, stwo, 600,  1); // s2 90cm
    ...
    //rc = ctrl3_check_front_sblks(0, &tvars, tconf, 0, &rettrigs, 0);
    int rc = _lt4_get_trigs(0, &tvars, tconf, 0, &rettrigs, 0);
    XCTAssert(rc==0);
...
- (void) testStartNoOccTurnout
{
    ...
    topology_set_turnout(to1, 1, -1);
    ctrl3_init_train(0, &tvars, stwo, 620, 1); // s2 90cm
    //tvars._curposmm = 620;
    /*ctrl3_get_next_sblks(0, &tvars, tconf);
    XCTAssert(tvars.rightcars.numlsblk == 1);
    static const int exp2[] = { 1 };
...
- (void) testStartNoOccTurnout2
{
    ...
    // same than previous, but to1 disallowed going forward
    topology_set_turnout(to1, 1, -1);
    ctrl3_init_train(0, &tvars, stwo, 750, 1); // s2 90cm
    //tvars._curposmm = 750;
    /*ctrl3_get_next_sblks(0, &tvars, tconf);
    XCTAssert(tvars.rightcars.numlsblk == 1);
    static const int exp2[] = { 1 };
...
- (void) testLeft1
{
    ...
    lsblk_num_t r2[4] = {-1,-1,-1,-1};
    tconf->trainlen_left_cm = 2;
    n = ctrl3_get_next_sblks_(0, &tvars, tconf, 1, r2, 4, NULL);
    XCTAssert(n==0);
    
    
    lsblk_num_t r3[4] = {-1,-1,-1,-1};
    tconf->trainlen_left_cm = 80;
    n = ctrl3_get_next_sblks_(0, &tvars, tconf, 1, r3, 4, NULL);
    XCTAssert(n==1);
...
- (void) testProgressLeft // KO20230330
{
    ...
    topology_set_turnout(to0, topo_tn_turn, -1);
    topology_set_turnout(to1, topo_tn_turn, -1);
    ctrl3_init_train(0, &tvars, sone, (45-4)*10, 1); // s1 45cm
     ...
     */
    rettrigs_t rettrigs = {0};
    int rc = _lt4_get_trigs(0, &tvars, tconf, 1, &rettrigs, 0);
    XCTAssert(rc==0);
    ...
    tvars.c1_sblk = next_lsblk(tvars.c1_sblk, 1, NULL);
    /*rc = ctrl3_get_next_sblks(0, &tvars, tconf);
