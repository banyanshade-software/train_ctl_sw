...
#define 	CODE_CVAL			0xC8
#define 	CODE_STRVAL			0xC9
...
static const uint8_t default_layout[] = {
		...
		CODE_ZONE_MODE, CODE_STR|1,
		CODE_ZONE_TEXT1, CODE_STR|2,CODE_CVAL, 0, CODE_UVAL, 1,
		CODE_ZONE_TEXT2, CODE_STRVAL, 2,
		CODE_END
};
...
void disp_layout(int numdisp)
{
	...
	for (int i=0; i<MAX_OPCODE_PER_DISPLAY; i++) {
		...
		case CODE_CVAL:
			i++;
			v16u = _GET_REG(numdisp, d[i]);
			ssd1306_WriteChar(v16u, *curfont, White);
			break;
		case CODE_STRVAL:
			i++;
			v16u = _GET_REG(numdisp, d[i]);
			if (strbase) ssd1306_WriteString(strbase[v16u], *curfont, White);
			break;
