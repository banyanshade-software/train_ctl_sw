...
static lsblk_num_t next_lsblk_free(int tidx, lsblk_num_t blknum, uint8_t left, uint8_t *palternate)
{
    lsblk_num_t ns = next_lsblk(blknum, left, palternate);
    if (ns.n >= 0) {
        //  check occupency
...
void ctrl2_check_checkstart(int tidx, train_ctrl_t *tvar)
{
    ...
    switch (tvar->_state) {
        ...
        case train_blk_wait:
            ...
            if (tvar->_dir) break;
            uint8_t alternate;
            ns = next_lsblk_free(tidx, tvar->c1_sblk, (tvar->desired_speed<0),  &alternate);
            if (ns.n>=0) {
                ctrl2_set_state(tidx, tvar, train_running_c1);
                ctrl2_set_dir(tidx, tvar, SIGNOF0(tvar->desired_speed));
                //ctrl2_set_tspeed(tidx, tvar, SIGNOF0(tvar->desired_speed));
                ...
                if (alternate) {
                    ...
                    tvar->_dir = SIGNOF0(tvar->desired_speed);
                } else {
...
void ctrl2_update_c2(int tidx, train_ctrl_t *tvar, const train_config_t *tconf, int32_t *ppose0)
{
    ...
    if (tvar->_dir) {
        uint8_t alternate = 0;
        ns = next_lsblk_free(tidx, tvar->c1_sblk, (tvar->_dir < 0), &alternate);
...
int ctrl2_evt_pose_triggered(int tidx, train_ctrl_t *tvar, uint8_t ca_addr, uint8_t trigbits, int16_t cposd10)
{
    ...
    if (trigbits & (1<<0)) {
        if (tvar->trig_eoseg) {
            ...
            lsblk_num_t ns = next_lsblk_free(tidx, tvar->c1_sblk, (tvar->_dir<0), NULL);
            if (ns.n<0) {
...
t2:
    ...
    if (trigbits & (1<<1)) {
        itm_debug2(tidx, "bad trigbit", tidx, trigbits);
        fatal();
    }
    return retcode;
}
